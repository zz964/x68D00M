...
/* */
/* C version: always compiled. Calls R_MapPlane internally. */
#ifndef HAVE_ASM_PLANES
void R_DrawPlanes (void)
{
	...
	for (pl = visplanes; pl < lastvisplane; pl++)
	{
		...
		/* Cache the previous column's top/bottom rather than re-reading
		 * pl->top[x-1] and pl->bottom[x-1] every iteration.  Halves the
		 * number of array reads in this inner loop. */
		{
			int t1 = (int)pl->top[pl->minx - 1];
			int b1 = (int)pl->bottom[pl->minx - 1];
			for (x=pl->minx; x<= stop; x++)
			{
				int t2 = (int)pl->top[x];
				int b2 = (int)pl->bottom[x];
